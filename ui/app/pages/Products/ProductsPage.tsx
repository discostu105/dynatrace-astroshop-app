...
/**
 * Helper function to check if a product matches the current filter criteria.
 * Centralizes filter logic to make it easier to maintain and test.
 */
const matchesFilters = (
  ...
  // Status filter (in-stock, out-of-stock, or all)
...
export const ProductsPage = () => {
  ...
  // Compute statistics from the full product list (always shows total inventory stats, not filtered counts)
  const stats = useMemo(
    () => ({
      total: MOCK_PRODUCTS.length,
      available: MOCK_PRODUCTS.filter((p) => p.stock > 0).length,
      outOfStock: MOCK_PRODUCTS.filter((p) => p.stock === 0).length,
    }),
    [],
  );

  // Filter products based on current filter state (memoized to avoid re-filtering on every render)
  const filteredProducts = useMemo(() => {
    return MOCK_PRODUCTS.filter((product) =>
      matchesFilters(product, { searchTerm, category, status }),
    );
